...
import { useRouter } from "next/navigation"; // <-- Giữ lại router
import { authService } from "@/services/authService"; // <-- Giữ lại authService
...
export function LoginPage() {
  const { login, setAuthData } = useAuth(); // Vẫn cần cả login và setAuthData
  ...
  // Hàm login bằng email/pass giữ nguyên
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    try {
      await login({ identifier: email, password });
      // AuthContext tự chuyển hướng
    } catch (err: any) {
      setIsLoading(false);
    }
  };

  // --- BƯỚC 2: TẠO HÀM signInWithGoogle DÙNG FIREBASE ---
  const signInWithGoogle = async () => {
    setIsLoading(true); // Bật loading
    const provider = new GoogleAuthProvider(); // Tạo provider của Firebase
    try {
      // 1. Mở pop-up đăng nhập Google của Firebase
      const result = await signInWithPopup(auth, provider);
      const user = result.user;

      // 2. Lấy idToken từ Firebase (Token này backend sẽ hiểu)
      const idToken = await user.getIdToken();

      // 3. Gửi idToken này đến backend của bạn (API backend không đổi)
      ...
      if (backendUser && backendToken) {
        ...
        // Trường hợp hiếm: Backend trả về 200 OK nhưng không có user/token
      ...
      if (err.response?.status === 404) {
        ...
        try {
          // Lấy lại thông tin từ Firebase Auth sau khi pop-up thành công
        ...
        const errMsg =
          ...
          err.message || // Lỗi từ Firebase (ví dụ: network error)
