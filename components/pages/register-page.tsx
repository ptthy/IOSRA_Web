...
import { useAuth } from "@/context/AuthContext"; // <-- Giữ lại useAuth

// --- BƯỚC 1: IMPORT FIREBASE ---
import { GoogleAuthProvider, signInWithPopup } from "firebase/auth";
import { auth } from "@/lib/firebase"; // <-- Đảm bảo đường dẫn này đúng
// --- XÓA CÁC IMPORT KHÔNG CẦN NỮA ---
// import { GoogleLogin } from "@react-oauth/google"; // Xóa
// import { jwtDecode } from "jwt-decode"; // Xóa
// --------------------------------------------------

export function RegisterPage() {
  const router = useRouter();
  const { setAuthData } = useAuth(); // Vẫn cần setAuthData
  ...
  const [isLoading, setIsLoading] = useState(false); // Dùng chung state loading

  // Hàm đăng ký bằng email/pass giữ nguyên
  ...
  // --- BƯỚC 2: TẠO HÀM signInWithGoogle DÙNG FIREBASE (GIỐNG HỆT LOGIN) ---
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      const response = await authService.loginWithGoogle({ idToken }); // API backend vẫn giữ nguyên

      // 4. Xử lý response từ backend
      const { user: backendUser, token: backendToken } = response.data;
      if (backendUser && backendToken) {
        // CASE A: User ĐÃ TỒN TẠI -> Đăng nhập thành công
        setAuthData(backendUser, backendToken); // Context sẽ tự chuyển hướng về home
      } else {
        toast.error("Đăng nhập Google thất bại (BE không trả về user/token).");
        setIsLoading(false);
      }
    } catch (err: any) {
      // 5. Xử lý lỗi

      // --- SỬA Ở ĐÂY: KIỂM TRA LỖI 409 CONFLICT TỪ BACKEND ---
      // Kiểm tra xem có phải lỗi từ axios (có response) và status là 409 không
      if (
        err.response?.status === 409 &&
        err.response?.data?.error?.code === "AccountNotRegistered"
      ) {
        // --- (Bạn cũng có thể chỉ cần kiểm tra: if (err.response?.status === 409)) ---

        // CASE B: Lỗi 409 từ backend -> User mới, cần hoàn tất đăng ký
        try {
          const currentUser = auth.currentUser; // Lấy thông tin user vừa đăng nhập qua Firebase
          if (currentUser) {
            const idToken = await currentUser.getIdToken(); // Lấy lại idToken mới nhất
            const googleEmail = currentUser.email; // Lấy email từ Firebase Auth

            if (idToken && googleEmail) {
              toast.info("Tài khoản Google mới, vui lòng hoàn tất đăng ký.");
              // Chuyển hướng đến trang hoàn tất, đính kèm token và email
              router.push(
                `/google-complete?idToken=${idToken}&email=${googleEmail}`
              );
              // Lưu ý: Không tắt loading ở đây vì trang sẽ chuyển hướng đi
            } else {
              // Trường hợp hiếm: không lấy được token/email từ Firebase Auth
              toast.error("Không lấy được thông tin Google sau khi đăng nhập.");
              setIsLoading(false);
            }
          } else {
            // Trường hợp hiếm: không tìm thấy currentUser trong Firebase Auth
            toast.error(
          ...
          // Lỗi xảy ra trong quá trình lấy lại token/email
        ...
        // Người dùng tự đóng pop-up Firebase
        toast.info("Bạn đã đóng cửa sổ đăng nhập Google.");
        setIsLoading(false);
      } else {
        // Các lỗi khác (từ backend hoặc Firebase)
        const errMsg =
          err.response?.data?.message || // Lỗi từ Backend
          err.message || // Lỗi từ Firebase (ví dụ: network error, auth/account-exists-with-different-credential)
          "Đăng nhập Google thất bại.";
        toast.error(errMsg);
        setIsLoading(false);
      }
    }
    // Không cần setIsLoading(false) nếu thành công vì sẽ chuyển trang
  };
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-6">
            ...
            <Button
              ...
              onClick={signInWithGoogle} // <-- GỌI HÀM FIREBASE KHI CLICK
              disabled={isLoading}
            >
              ...
              Đăng ký với Google {/* <-- Sửa chữ cho khớp */}
            </Button>
            {/* ------------------------------------------- */}
