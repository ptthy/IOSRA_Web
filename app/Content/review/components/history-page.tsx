...
const translateLength = (length?: string) => {
  ...
  const translateAIResult = (result?: string) => {
    ...
    if (lower === "approved") return "Được duyệt";
    if (lower === "rejected") return "Bị từ chối";
    if (lower === "flagged") return "Cần xem xét";
    return result; // Trả về nguyên gốc nếu không khớp
  };
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      // Filter Status
      if (filterStatus !== "all") {
        if (filterStatus === "published" && item.status !== "published")
          return false;
        if (filterStatus === "rejected" && item.status !== "rejected")
          return false;
      }
      // Filter Time
      if (filterTime !== "all") {
        ...
        const now = moment();
        if (filterTime === "today" && !itemDate.isSame(now, "day"))
          return false;
        if (
          filterTime === "7days" &&
          itemDate.isBefore(now.subtract(7, "days"))
        )
          return false;
        if (
          filterTime === "30days" &&
          itemDate.isBefore(now.subtract(30, "days"))
        )
          return false;
      }
      return true;
  ...
  const stats = useMemo(() => {
    ...
    const published = filteredData.filter(
    ...
    <div className="min-h-screen bg-[var(--background)] p-8 transition-colors duration-300">
      ...
      >
        ...
        <p className="text-[var(--muted-foreground)]">
          Theo dõi các quyết định duyệt Truyện và Chương
        </p>
      </motion.div>
      ...
      >
        {stats.map((s, i) => (
          <Card
            key={i}
            className="p-5 border border-[var(--border)] bg-[var(--card)] rounded-xl shadow-sm"
          >
            <p className={`text-3xl font-semibold mb-1 ${s.color}`}>
              {s.value}
            </p>
        ...
        >
          <TableHeader>
            <TableRow className="bg-[var(--muted)]/20">
              ...
              <TableHead className="py-4 px-6 w-[160px]">
                Trạng thái / AI
              </TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell
                  colSpan={5}
                  className="h-48 text-center text-red-600"
                >
              ...
              <TableRow>
                <TableCell
                  colSpan={5}
                  className="h-48 text-center text-[var(--muted-foreground)]"
                >
              ...
              filteredData.map((item) => (
                <TableRow
                  key={item.id}
                  className="border-b hover:bg-[var(--muted)]/20 transition"
                >
                  <TableCell className="py-4 px-6 text-[var(--muted-foreground)]">
                    <div className="flex flex-col">
                      <span className="font-medium text-[var(--foreground)]">
                        {moment(item.submittedAt).format("HH:mm")}
                      </span>
                      <span className="text-xs">
                        {moment(item.submittedAt).format("DD/MM/YYYY")}
                      </span>
                    </div>
                  </TableCell>
                      ...
                      {item.aiScore !== undefined && (
                        <span
                          className={cn(
                            ...
                            getScoreColor(item.aiScore)
                          )}
                        >
                          <Bot className="w-3 h-3" /> Điểm AI: {item.aiScore}/10
                        </span>
                      )}
                    </div>
                  </TableCell>
        ...
        <DialogContent className="bg-[var(--card)] border border-[var(--border)] text-[var(--foreground)] sm:max-w-[700px] p-0 overflow-hidden gap-0 flex flex-col max-h-[90vh]">
          <DialogHeader className="px-6 py-4 border-b border-[var(--border)] bg-[var(--muted)]/10 shrink-0">
            ...
            >
              ...
              {selectedItem?.status === "rejected"
                ? "Quyết định: Từ chối"
                : "Quyết định: Đã xuất bản"}
            </DialogTitle>
            <DialogDescription className="text-xs mt-1 font-mono flex gap-3">
              <span>Mã Review: {selectedItem?.id.slice(0, 8)}...</span>
              <span>•</span>
              <span className="uppercase">
                {selectedItem?.type === "story" ? "Truyện" : "Chương"}
              </span>
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedItem && (
            <div className="overflow-y-auto p-6 space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="flex-1 space-y-3 text-sm">
                  ...
                  <div className="grid grid-cols-[110px_1fr] gap-2 items-baseline">
                    <span className="text-[var(--muted-foreground)] text-xs uppercase tracking-wider">
                      Tiêu đề
                    </span>
                  ...
                  {/* STORY SPECIFIC: Length Plan (ĐÃ DỊCH) */}
                  {selectedItem.type === "story" && selectedItem.lengthPlan && (
                    <div className="grid grid-cols-[110px_1fr] gap-2 items-baseline">
                      ...
                      <Badge variant="outline" className="w-fit">
                        {translateLength(selectedItem.lengthPlan)}
                      </Badge>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-[110px_1fr] gap-2 items-baseline">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {moment(selectedItem.submittedAt).format(
                        "HH:mm - DD/MM/YYYY"
                      )}
                    </span>
                  </div>
                  ...
                  {selectedItem.type === "story" &&
                    ...
                    selectedItem.tags.length > 0 && (
                      <div className="grid grid-cols-[110px_1fr] gap-2 items-start mt-1">
                        <span className="text-[var(--muted-foreground)] text-xs uppercase tracking-wider pt-1">
                          Thể loại
                        </span>
              ...
              {/* 2. Phần AI Analysis (ĐÃ DỊCH TIÊU ĐỀ VÀ KẾT QUẢ) */}
                    ...
                    {selectedItem.aiResult && (
                      <span className="text-xs text-[var(--muted-foreground)]">
                        ...
                        <span className="font-medium text-[var(--foreground)] uppercase">
                          {translateAIResult(selectedItem.aiResult)}
                        </span>
                      </span>
                    )}
                    <Badge
                      ...
                      className={cn(
                        "border-0 font-bold",
                        getScoreColor(selectedItem.aiScore)
                      )}
                    >
                      Điểm:{" "}
              ...
              {selectedItem.type === "story" &&
                (selectedItem.description || selectedItem.outline) && (
