...
import { Loader2, EyeOff, CheckCircle, XCircle } from "lucide-react";
...
export function ReportActionModal({ report, isOpen, onClose, onSuccess }: ReportActionModalProps) {
  ...
  const handleResolve = async () => {
    if (!confirm("Xác nhận báo cáo ĐÚNG? User vi phạm sẽ bị tính 1 Strike.")) return;
    setLoading(true);
    try {
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2">
              ...
              <p className="bg-gray-100 p-3 rounded mt-1 text-gray-800 italic">
                "{report.details || 'Không có mô tả chi tiết'}"
              </p>
            </div>
            <div className="col-span-2">
              <span className="font-semibold text-gray-500 block">ID Nội dung (Target ID):</span>
              <code className="text-xs bg-gray-100 px-1 rounded">{report.targetId}</code>
            </div>
          </div>

          {/* Khu vực hành động với Nội dung */}
          <div className="border-t pt-4">
            <h4 className="text-sm font-semibold mb-3">Hành động với nội dung:</h4>
