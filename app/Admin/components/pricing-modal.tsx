...
import { Loader2, Save, History, DollarSign, Gem } from "lucide-react"; // Thêm Gem ở đây

// Import API đã cập nhật
import { 
  pricingApi, 
  ChapterRule, 
  VoiceRule, 
...
export default function PricingModal() {
  ...
  // Định nghĩa Icon Gem để tái sử dụng
  const GemIcon = () => (
    ...
    <div className="p-6 space-y-6 font-sans">
      <div className="flex justify-between items-end">
        ...
        <Button variant="outline" onClick={fetchData}>
          <History className="w-4 h-4 mr-2" /> Làm mới
        </Button>
      </div>
        ...
        <TabsContent value="chapters">
          <Card className="border-none shadow-md">
            <CardHeader>
              ...
              <CardDescription>Chi phí người đọc trả để mở khóa chương dựa trên số chữ.</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
        ...
        <TabsContent value="voice">
          <Card className="border-none shadow-md">
            <CardHeader>
              ...
              <CardDescription>Chi phí tác giả bỏ ra để tạo giọng đọc cho chương truyện.</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Số ký tự</TableHead>
                    <TableHead>Giá mua Voice (<GemIcon />)</TableHead>
                    <TableHead>Giá tạo Audio (<GemIcon />)</TableHead>
                    <TableHead className="text-right">Hành động</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                ...
                {topups.map((tp) => (
                  <div key={tp.pricing_id} className="p-4 border rounded-xl space-y-3 bg-muted/30">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge color={tp.is_active ? "green" : "red"}>{tp.is_active ? "Active" : "Inactive"}</Badge>
                    </div>
                    ...
                    >
                      {updating === tp.pricing_id ? <Loader2 className="animate-spin" /> : "Cập nhật gói"}
                    </Button>
                  </div>
                ))}
              </div>
...
function Badge({ children, color }: { children: React.ReactNode, color: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${colorClass}`}>{children}</span>;
}